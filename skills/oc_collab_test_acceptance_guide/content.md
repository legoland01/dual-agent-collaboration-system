# OC-Collab 测试验收指南 (v2.2.6)

## 快速参考

### 角色分工

| 角色 | 必须做 | 禁止做 |
|------|--------|--------|
| **Agent 1** | 黑盒测试、验收签署、**编写测试代码** | 产品代码实现 |
| **Agent 2** | 产品代码实现、白盒测试、开发签署 | 黑盒测试、签署"验收通过" |

**说明**：
- Agent1可以编写测试代码（tests/目录下的*.py文件）
- Agent1禁止修改产品代码（src/目录下的代码）
- 测试代码 = 验证产品功能的代码，不涉及产品逻辑实现

### 测试时机

```
需求: APPROVED ✅
设计: APPROVED ✅
开发: ⏳ pending → ✅ completed  ← ← ← 此时才能开始测试！
测试: ⏳ pending
验收: ⏳ pending
```

---

## 测试前检查清单

### 1. 开发状态检查 ⭐

**在运行任何测试前，必须确认：**

```
bash
# 1. 确认开发已完成
cat state/project_state.yaml | grep "v2.2.3" -A5

# 应该看到:
#   development:
#     status: completed  # 而不是 pending
```

**禁止**: 开发状态为 pending 时运行测试

### 2. 代码提交检查

```bash
# 1. 确认所有代码已提交
git status

# 2. 确认有开发完成的commit
git log --oneline -5

# 3. 确认模块文件存在
ls -la src/core/
# 应该看到:
#   - context_manager.py
#   - todo_sync_manager.py
```

**禁止**: 有未提交的代码时运行测试

### 3. 版本核对

```bash
# 1. 确认测试文件版本与开发版本一致
grep "v2.2.3" tests/test_v2_2_3.py

# 2. 确认设计文档版本
grep "版本.*2.2.3" docs/02-design/DETAIL-*.md

# 3. 核对需求版本
grep "v2.2.3" docs/01-requirements/requirements_v2.2.3_READY.md
```

### 4. 环境准备

```bash
# 1. 拉取最新代码（如果有远程更新）
git pull

# 2. 确认依赖已安装
pip install -e .

# 3. 清除旧缓存
rm -rf .pytest_cache __pycache__ src/*/__pycache__
```

---

## 测试准备流程

### Agent 1 的测试准备工作

| 步骤 | 操作 | 状态 |
|------|------|------|
| 1 | 阅读设计文档，理解模块接口 | ☐ |
| 2 | 编写/完善测试用例 | ☐ |
| 3 | 确认开发状态为 completed | ☐ |
| 4 | 确认代码已提交 | ☐ |
| 5 | 核对版本号一致 | ☐ |
| 6 | 运行测试 | ☐ |
| 7 | 签署验收 | ☐ |

### 测试用例来源

1. **设计文档中的测试用例清单**
   - 位置: `docs/02-design/DETAIL-*.md` → `## 4. 测试设计`
   
2. **已有测试文件**
   - 位置: `tests/test_v2_2_3.py`
   
3. **黑盒测试用例模板**
   - 位置: `docs/03-test/blackbox_test_cases_TEMPLATE.md`

---

## 测试执行规范

### 运行测试

```bash
# 运行特定版本测试
python -m pytest tests/test_v2_2_3.py -v

# 运行所有测试（回归测试）
python -m pytest tests/ -v --tb=short

# 运行黑盒测试
python -m pytest tests/test_blackbox_*.py -v
```

### 测试结果记录

| 结果类型 | 处理方式 |
|----------|----------|
| ✅ 全部通过 | 签署验收，更新状态 |
| ⚠️ 部分失败 | 记录问题，创建TODO分配给Agent2修复 |
| ❌ 全部失败 | 检查环境，报告Bug |

---

## ⚠️ 发现Bug后的处理流程

**绝对禁止**: 发现Bug后标记验收完成

**正确流程**:

```
发现Bug
  ↓
  1. 创建Bug报告文件 (docs/00-memos/BUG-YYYYMMDD-XXX_XXX.md)
  ↓
  2. 创建TODO任务分配给Agent2修复
  ↓
  oc-collab todowrite --content "修复 BUG-YYYYMMDD-XXX" --priority P0 --agent 2
  ↓
  3. 更新 project_state.yaml
     - status: 保持 "in_progress"
     - phase: 保持 "testing"
     - bugs: 添加Bug列表
  ↓
  4. 提交变更
  ↓
  ⏳ 等待Agent2修复并提交
  ↓
  5. Agent1看到修复后，自主创建验收TODO
  ↓
  6. 重新测试 → 通过 → 签署验收
```

### ⚠️ TODO创建规则

| 规则 | 说明 |
|------|------|
| 只创建"修复Bug"的TODO | ✅ 分配给Agent2 |
| 不要提前创建"重新测试"的TODO | ❌ 等待Agent2修复后再创建 |
| 不要代Agent1创建TODO | ❌ Agent1看到修复后自主创建 |

**反面教训** (BUG-20260208-001):

| 错误做法 | 正确做法 |
|----------|----------|
| 发现Bug后标记验收完成 | 保持 testing 状态 |
| 只写Bug报告不分配任务 | 必须创建TODO分配给Agent2 |
| Bug只在docs/记录 | 同时更新project_state.yaml |
| 让Agent2自己找Bug | 主动分配任务 |

---

## 验收签署

### 签署前提

**只有所有测试通过且无Bug时，才能签署验收**

```
验收条件:
- ✅ 单元测试: 全部通过
- ✅ E2E测试: 全部通过  
- ✅ 黑盒测试: 全部通过
- ✅ CLI功能: 全部实现
- ❌ 无未修复的Bug
```

### 签署内容模板

```
验收结果: ✅ 通过

测试执行:
- 测试文件: tests/test_v2_2_3.py
- 测试命令: python -m pytest tests/test_v2_2_3.py -v
- 测试结果: XX passed, YY failed

问题记录:
- [P0]: xxx
- [P1]: xxx

签署: Agent 1 @ 2026-02-08
```

### ⚠️ 有Bug时不签署

```
验收结果: ❌ 未完成

测试执行:
- 测试命令: python -m pytest tests/test_v2_2_3.py -v
- 测试结果: XX passed, YY failed

发现问题:
- [P0]: CLI命令未完全实现 (BUG-20260208-001)

后续处理:
- TODO-052: Agent2 修复Bug
- 待修复后重新测试
```

### 签署规范

| 角色 | 正确的签署内容 |
|------|---------------|
| Agent 1 | "测试验收通过" / "验收通过" |
| Agent 2 | "开发完成" / "测试环境就绪" |

---

## 常见错误

| 错误 | 正确做法 |
|------|---------|
| 开发pending时运行测试 | 等待开发状态变为completed |
| 不检查git status直接测试 | 先确认代码已提交 |
| 不核对版本号 | 确认测试文件与开发版本一致 |
| 测试失败直接签署 | 必须记录问题，要求修复 |
| Agent1做白盒测试 | 白盒测试由Agent2执行 |
| 发现Bug后标记验收完成 | 创建TODO分配给Agent2，保持testing状态 |
| 只写Bug报告不分配任务 | 必须创建TODO分配给Agent2 |
| Bug只在docs/记录 | 同时更新project_state.yaml |

---

## Git 提交规范 ⭐

**测试完成后，必须提交测试报告**

```bash
# 1. 提交测试用例更新
git add tests/
git commit -m "test: 更新 v2.2.3 测试用例"
oc-collab sync-all -m "test: 更新 v2.2.3 测试用例"

# 2. 验收通过后，提交验收报告
git add docs/03-test/
git commit -m "docs: v2.2.3 测试验收通过"
oc-collab sync-all -m "docs: v2.2.3 测试验收通过"
```

**禁止**: 测试完成后不提交，积压多个提交

---

## 文档同步环节 ⭐

**验收完成后、版本发布前，必须执行文档同步**

```
验收 ✅ → Agent2签署 ⏳ → 文档同步 → 部署/发布
```

### Agent 1 文档同步清单

| 检查项 | 位置 | 操作 |
|--------|------|------|
| Skill更新 | `skills/` | 确保Skill反映新功能 |
| CLI帮助 | `python -m src.cli.main --help` | 检查命令描述 |
| README | `README.md` | 更新功能列表 |
| API文档 | `docs/` | 补充新功能说明 |

### 文档同步步骤

```bash
# 1. 检查新功能是否需要Skill
ls skills/
# 如果有新功能，创建/更新Skill

# 2. 验证CLI命令帮助
python -m src.cli.main --help | grep -E "new|feature"
# 确保新命令有正确描述

# 3. 检查README
grep -E "v2\.2\.[0-9]" README.md
# 确保版本号和功能列表最新
```

### 文档同步输出模板

```
文档同步完成:

检查项:
- ✅ skills/: 已更新
- ✅ CLI帮助: 已验证
- ✅ README: 已更新

新文档:
- skills/oc_collab_xxx/

更新文档:
- README.md (添加 v2.2.3 功能)

签署: Agent 1 @ 2026-02-08
```

---

## 版本历史

| 版本 | 日期 | 变更 |
|------|------|------|
| v1 | 2026-02-08 | 初始版本 |
| v2 | 2026-02-08 | 添加测试前检查清单，强调开发状态检查 |
| v3 | 2026-02-08 | 添加Bug处理流程（BUG-20260208-001教训） |
| v4 | 2026-02-08 | 添加文档同步环节（验收后、发布前） |
| v5 | 2026-02-09 | 添加SOP四要素结构（Phase2） |
| v6 | 2026-02-14 | 明确Agent1可以编写测试代码 |

---

## 📚 教训总结（v5新增）

### BUG-20260208-003: 测试中发现新Bug的处理流程

**场景**: v2.2.4功能测试通过后，发现oc-collab工具无法识别项目状态

**教训**:

| 错误做法 | 正确做法 |
|----------|----------|
| 测试通过就立即签署验收 | 完整测试所有功能，包括基础设施 |
| 发现Bug只写报告 | 必须编写测试用例覆盖Bug场景 |
| 测试用例只给Agent2写 | Agent1负责黑盒测试，可以写测试代码 |
| 签署后发现Bug再回退 | 测试中发现Bug直接回退验收状态 |

### 测试中发现Bug的完整流程

```
1. 执行测试 → 发现新Bug
2. ✅ 编写测试用例覆盖Bug（Agent1职责）
3. ✅ 验证测试失败（证明Bug存在）
4. ✅ 回退验收签署状态
5. ✅ 创建Bug报告
6. ✅ 创建TODO分配给Agent2
7. ✅ 提交变更（测试代码 + Bug报告）
8. ⏳ 等待Agent2修复
9. 修复后重新测试 → 通过 → 重新签署
```

### 关键检查点

| 检查点 | 说明 |
|--------|------|
| 测试完整性 | v2.2.x功能 + 基础设施（oc-collab） |
| 测试代码 | Agent1可以写测试代码 |
| 签署时机 | 所有测试通过后才能签署 |
| Bug覆盖 | 新发现的Bug必须有测试用例覆盖 |

---

**维护者**: Agent 1
**更新日期**: 2026-02-08

---

## SOP结构概览 ⭐

| SOP要素 | 内容 |
|---------|------|
| **1. 触发条件** | 见"测试时机"章节 |
| **2. 操作步骤** | 见"测试前检查清单"到"签署规范" |
| **3. 输出产物** | 测试报告、签署记录 |
| **4. 验收标准** | 见"4. 签署规范" |

---

## 1. 触发条件 ⭐

**触发关键词**: `development_completed`, `agent_asks_test`, `before_deployment`

| 场景 | 触发条件 |
|------|----------|
| 开发完成 | development.status: completed |
| Agent询问测试 | 提供测试规范 |
| 部署前 | 验收签署 |

**参考**: "测试时机"章节

## 2. 操作步骤 ⭐

| 阶段 | 步骤 | 操作 |
|------|------|------|
| 测试前 | 1 | 确认开发状态为completed |
| 测试前 | 2 | 确认代码已提交 |
| 测试前 | 3 | 核对版本号一致 |
| 测试前 | 4 | 准备测试环境 |
| 测试执行 | 5 | 运行测试（单元/E2E/黑盒） |
| 测试执行 | 6 | 记录测试结果 |
| Bug处理 | 7 | 发现Bug时创建报告和TODO |
| 验收 | 8 | 所有测试通过后签署验收 |

参考详细步骤：
- "测试前检查清单"
- "测试执行规范"
- "发现Bug后的处理流程"
- "验收签署"

## 3. 输出产物 ⭐

| 产物 | 位置 | 格式 |
|------|------|------|
| 黑盒测试用例 | docs/03-test/blackbox_test_*.md | Markdown |
| 测试报告 | docs/03-test/*.md | Markdown |
| 签署记录 | 需求/设计文档末尾 | Markdown表格 |

## 4. 验收标准 ⭐

| 标准 | 检查方法 |
|------|----------|
| 单元测试全部通过 | python3 -m pytest tests/ -v |
| E2E测试全部通过 | python3 -m pytest tests/test_e2e*.py -v |
| 状态同步验证通过 | python3 -m pytest tests/test_state_sync.py -v |
| 代码覆盖率 >=80% | pytest --cov |
| 无未修复Bug | project_state.yaml 检查 |

---

## 4.5 状态同步验证测试 ⭐

### 背景

v2.2.11 StateReceiver 问题暴露：StateReceiver 使用 `state/state_queue.json`，CLI 使用 `state/todo_queue.yaml`，两个队列独立。

### 验证要求

**所有涉及多模块协作的功能，必须包含状态同步验证测试：**

| 测试类型 | 必须包含 | 验证方法 |
|----------|----------|----------|
| 队列同步 | ✅ | 发送方 → 接收方 → CLI 查询 |
| 数据一致性 | ✅ | 比较两个队列的数据 |
| 端到端链路 | ✅ | 完整业务流程验证 |

### 状态同步测试用例模板

```markdown
### TC-SYNC-XXX: [场景名称]

**验证目标**: 确认 [源] → [目标] 的数据同步

**测试步骤**:
1. [操作1]
2. [操作2]
3. [验证1]

**验收标准**:
- [结果1]
- [结果2]
```

### 检查清单

创建测试用例前必须确认：

- [ ] 数据存储位置已明确（哪些文件）
- [ ] 数据流向已标注（谁写入、谁读取）
- [ ] 同步路径已定义（如果需要同步）
- [ ] 验证方法已确定（如何确认同步成功）

### 常见错误

| 错误 | 描述 | 解决方案 |
|------|------|----------|
| 队列独立 | StateReceiver 和 CLI 使用不同队列 | 实现同步逻辑 |
| 无验证 | 只测试接收，不测试显示 | 添加 CLI 查询验证 |
| 单向测试 | 只测试发送或只测试接收 | 完整链路测试 |

---

## 5. 验收TODO规范 ⭐

### 5.1 验收前提

**只有所有测试通过且无Bug时，才能签署验收**

```
验收条件:
- ✅ 单元测试: 全部通过
- ✅ E2E测试: 全部通过  ← 必须做！
- ✅ 黑盒测试: 全部通过  ← 必须做！
- ✅ CLI功能: 全部实现
- ❌ 无未修复的Bug
```

### 5.2 禁止行为

| ❌ 禁止 | 说明 |
|---------|------|
| 跳过测试直接签署 | E2E/黑盒测试必须执行 |
| 只做单元测试就验收 | E2E端到端测试必须通过 |
| 知道有Bug还签署 | 必须修复后再验收 |
| 测试失败还通过 | 测试必须全部通过 |

### 5.3 验收TODO模板

```yaml
验收TODO内容必须包含:
- 测试类型清单: 单元测试/E2E测试/黑盒测试
- 验收标准: 各项测试的通过条件
- 执行人: Agent2(开发侧) / Agent1(验收侧)
```

### 5.4 正确流程

```
开发完成 → Agent2执行E2E测试 → 通过 → 提交验收
                                    ↓ 不通过
                               Agent2修复Bug
                                    ↓
                               重新测试 → 通过
                                    ↓
                               Agent1执行黑盒测试 → 通过 → 签署验收
```

### 5.5 错误流程（禁止）

```
开发完成 → 跳过测试 → Agent1直接签署验收  ❌ 禁止！
开发完成 → 只做单元测试 → 签署验收  ❌ 禁止！
```

---

## 版本历史

| 版本 | 日期 | 变更 | 作者 |
|------|------|------|------|
| v1.0.1 | 2026-02-14 | 新增"验收TODO规范"章节 | Agent 1 |
| v1.0.2 | 2026-02-14 | 新增"状态同步验证测试"章节 | Agent 2 |
