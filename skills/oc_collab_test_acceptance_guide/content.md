...
| 角色 | 必须做 | 禁止做 |
|------|--------|--------|
| **Agent 1** | 黑盒测试、验收签署、**编写测试代码** | 产品代码实现 |
| **Agent 2** | 产品代码实现、白盒测试、开发签署 | 黑盒测试、签署"验收通过" |

**说明**：
- Agent1可以编写测试代码（tests/目录下的*.py文件）
- Agent1禁止修改产品代码（src/目录下的代码）
- 测试代码 = 验证产品功能的代码，不涉及产品逻辑实现
...
| v5 | 2026-02-09 | 添加SOP四要素结构（Phase2） |
| v6 | 2026-02-14 | 明确Agent1可以编写测试代码 |

---

## 📚 教训总结（v5新增）

### BUG-20260208-003: 测试中发现新Bug的处理流程

**场景**: v2.2.4功能测试通过后，发现oc-collab工具无法识别项目状态

**教训**:

| 错误做法 | 正确做法 |
|----------|----------|
| 测试通过就立即签署验收 | 完整测试所有功能，包括基础设施 |
| 发现Bug只写报告 | 必须编写测试用例覆盖Bug场景 |
| 测试用例只给Agent2写 | Agent1负责黑盒测试，可以写测试代码 |
| 签署后发现Bug再回退 | 测试中发现Bug直接回退验收状态 |

### 测试中发现Bug的完整流程

```
1. 执行测试 → 发现新Bug
2. ✅ 编写测试用例覆盖Bug（Agent1职责）
3. ✅ 验证测试失败（证明Bug存在）
4. ✅ 回退验收签署状态
5. ✅ 创建Bug报告
6. ✅ 创建TODO分配给Agent2
...
| 检查点 | 说明 |
|--------|------|
| 测试完整性 | v2.2.x功能 + 基础设施（oc-collab） |
| 测试代码 | Agent1可以写测试代码 |
| 签署时机 | 所有测试通过后才能签署 |
| Bug覆盖 | 新发现的Bug必须有测试用例覆盖 |
...
| 阶段 | 步骤 | 操作 |
|------|------|------|
| 测试前 | 1 | 确认开发状态为completed |
| 测试前 | 2 | 确认代码已提交 |
| 测试前 | 3 | 核对版本号一致 |
| 测试前 | 4 | 准备测试环境 |
| 测试执行 | 5 | 运行测试（单元/E2E/黑盒） |
| 测试执行 | 6 | 记录测试结果 |
| Bug处理 | 7 | 发现Bug时创建报告和TODO |
| 验收 | 8 | 所有测试通过后签署验收 |
...
| 产物 | 位置 | 格式 |
|------|------|------|
| 黑盒测试用例 | docs/03-test/blackbox_test_*.md | Markdown |
| 测试报告 | docs/03-test/*.md | Markdown |
| 签署记录 | 需求/设计文档末尾 | Markdown表格 |

## 4. 验收标准 ⭐

| 标准 | 检查方法 |
|------|----------|
| 单元测试全部通过 | python3 -m pytest tests/ -v |
| E2E测试全部通过 | python3 -m pytest tests/test_e2e*.py -v |
| 代码覆盖率 >=80% | pytest --cov |
| 无未修复Bug | project_state.yaml 检查 |
