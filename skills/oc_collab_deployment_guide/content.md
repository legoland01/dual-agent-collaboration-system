...
**部署前必须完成文档同步**，否则禁止进入版本升级阶段。

```bash
# 文档同步检查清单
# 1. CHANGELOG.md - 添加版本发布记录
# 2. README.md - 更新功能列表和命令说明
# 3. CLI帮助 - 验证命令描述准确性
```
...
| 步骤 | 检查项 | 命令 |
|------|--------|------|
| **0** | **文档已同步** | `grep "v{x}.{y}.{z}" CHANGELOG.md` |
...
| 阶段 | 步骤 | 操作 | 强制 |
|------|------|------|------|
| **部署前** | **0** | **文档同步（CHANGELOG + README）** | ✅ **强制** |
| 部署前 | 1 | 确认测试验收通过 | |
| 部署前 | 2 | 升级版本号 | |
| 部署中 | 3 | 构建包并上传PyPI | |
| 部署中 | 4 | 推送Git并打标签 | |
...
| v1.1.2 | 2026-02-14 | 添加文档同步步骤（BUG-20260214-001） | Agent 1 |
