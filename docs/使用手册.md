...
### 4.7.3 Bug自动触发Agent 2修复

当测试阶段发现 bug 时，系统会自动触发 Agent 2 激活并回退到开发阶段。

```bash
# 测试阶段发现 bug 后，运行 auto 命令
oc-collab auto --force
```

**触发条件：**
- 项目阶段为 `testing`
- `test.issues_to_fix` 字段不为空

**自动执行的操作：**
1. 检测到 bug 列表
2. 激活 Agent 2
3. 将阶段从 `testing` 回退到 `development`
4. 记录历史
...
测试阶段发现 bug:
  1. Agent 1 更新测试结果:
     oc-collab project update --type test --cases 22 --passed 20
     # 系统自动记录 issues_to_fix = ["BUG-001: 问题1", "BUG-002: 问题2"]
